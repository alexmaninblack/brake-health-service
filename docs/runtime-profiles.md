...
The private advisory journal records before/after state and the optional exact
fact. Recovery accepts only the recorded before/after state; conflicts remain
...
overflow/non-enqueue, conflict quarantine and v1 no-model separation.

This is a source/host-tested checkpoint, not a successful deployment claim.
The pinned Linux ARM64/gRPC compile, real KAC/Service-identity mount ownership,
...
by Demo Control. Recovery fault-injection coverage is being extended before
claiming the complete v3 product qualification. No Docker, signing, publication,
